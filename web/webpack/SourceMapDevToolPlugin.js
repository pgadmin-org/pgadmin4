			...
			compilation.plugin("after-optimize-chunk-assets", function(chunks) {
				...
				tasks.forEach(function(task) {
					...
					if (sourceMap.sources) {
						sourceMap.sources = sourceMap.sources.filter(
							(content, idx) => {
								const module = modules[idx],
									mi = module.portableId;

								if (
									!(!mi && (
										typeof module === 'string' && module.startsWith(
											'webpack\/bootstrap'
										)
									))	&&
										!(mi && (
											mi.match(
												/^delegated ".+" from dll-reference [a-zA-Z]+$/
											) || mi.startsWith('external '))
										)
								) {
									console.log(`[SourceMap] ${mi || module}`);
									if(sourceMap.sourcesContent && !options.noSources) {
										sourceMapContents.push(
											`${sourceMap.sourcesContent[idx]}\n\n\n${ModuleFilenameHelpers.createFooter(modules[idx], requestShortener)}`
										);
									}
									return true;
								}
					...
					if(sourceMap.sourcesContent && !options.noSources) {
							sourceMap.sourcesContent = sourceMapContents;
					} else {
