...
import {
  ...
  expectNoDivergence, ensureServerRegistered, navigateToCatalogNode,
  openCreateDialog,
} from './audit-helpers';
...
test('Create Table dialog (TableSchema + ColumnSchema)', async ({ page }) => {
  ...
  const name = await ensureServerRegistered(page);
  await navigateToCatalogNode(page, name, 'Tables');

  await openCreateDialog(page, 'Tables', 'Table...');
  await page.getByRole('textbox', { name: 'Name' }).first().waitFor({
  ...
  await page.locator('button[data-test="Columns"]').click();
  await page.waitForTimeout(300);
...
test('Create Function dialog (FunctionSchema)', async ({ page }) => {
  ...
  const name = await ensureServerRegistered(page);
  await navigateToCatalogNode(page, name, 'Functions');

  await openCreateDialog(page, 'Functions', 'Function...');
  await page.getByRole('textbox', { name: 'Name' }).first().waitFor({
    state: 'visible', timeout: 20_000,
  });
  ...
  const argsTab = page.locator('button[data-test="Arguments"]');
  ...
  // Options tab
  const optsTab = page.locator('button[data-test="Options"]');
  ...
  const paramsTab = page.locator('button[data-test="Parameters"]');
