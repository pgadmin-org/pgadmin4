...
class PgtTaskGetStatsTestCase(BaseTestGenerator):
    """This class will test the get pgTimetable chain task stats API"""
    scenarios = utils.generate_scenarios("pgt_task_get_statistics",
    ...
    def setUp(self):
        ...
        name = "test_chain_get_stats%s" % str(uuid.uuid4())[1:8]
        self.chain_id = pgt_utils.create_pgtimetable_chain(self, name)

        task_name = "test_task_get_stats%s" % str(uuid.uuid4())[1:8]
        self.task_id = pgt_utils.create_pgtimetable_task(
            self, task_name, self.chain_id)

    def runTest(self):
        """This function will get pgTimetable chain task stats"""

        if self.is_positive_test:
            response = tasks_utils.api_get_stats(self)
