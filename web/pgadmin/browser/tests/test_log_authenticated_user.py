...
class _StubResponse:
    ...
    The headers are a real werkzeug Headers instance rather than a dict, so
    that values which Werkzeug would reject (control characters and the like)
    fail here in the same way they would on a live response.
    """
...
class TestHeaderOnLiveLogoutRequest(BaseTestGenerator):
    """End to end check that the two hooks are actually wired into the
    request cycle: a real GET /logout must still report the user who made
    it, which is the regression this all exists to prevent."""
    ...
    def runTest(self):
        self.app.config['LOG_AUTHENTICATED_USER'] = True
        try:
    ...
    def tearDownClass(cls):
        """Log the test client back in for the tests that follow."""
